...
class TSPState:
    def __init__(self, cost, path, table, depth, cityIndex, cities):
        self.cost = cost
        self.path = path  # path so far list of city ids
        self.table = table  # cost from city to city 2d numpy array
        self.depth = depth
        self.cityIndex = cityIndex
        self.cities = cities
        self.length = len(cities)

    def __lt__(self, other):  # does it have a higher priority
    ...
    def reduce(self):
        mins = np.amin(self.table, axis=1)  # row
        ...
        colMins = np.amin(self.table, axis=0)  # col
